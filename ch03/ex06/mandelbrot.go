...
func resampleQuad(srcImg image.Image, width int, height int) image.Image {
...
func average(x int, y int, s image.Image) color.Color {
	return average9(x, y, s)
}

func average4(x int, y int, s image.Image) color.Color {
	var r, g, b uint32
...
func average9(x int, y int, s image.Image) color.Color {
	var r, g, b uint32
...
func drawQuadrupleImg(w int, h int) image.Image {
...
func colorize(v complex128, z complex128, n uint8) color.Color {
	const contrast = 15
	blue := 255 - contrast*n
	red := 255 - blue
